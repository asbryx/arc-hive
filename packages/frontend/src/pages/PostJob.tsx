...
const JOB_TEMPLATES = [
  ...
  },
  {
    ...
    template: {
      ...
      description:
        'Write [Number] articles about [Topic]:\n- SEO optimized\n- 1000-2000 words each\n- Original research and insights\n- Include relevant keywords',
  ...
  if (!isConnected) {
    return (
      ...
      >
        ...
        <div style={{ fontSize: 14, color: 'var(--dim)', marginBottom: 24 }}>
    ...
    try {
      ...
      if (pendingListing) {
        ...
        createHash = await writeContractAsync({
          ...
          args: [zeroAddress, evaluatorAddr, expiredAt, onChainDesc, zeroAddress],
          chain: arcTestnet,
        })
        ...
        if (!jobCreatedLog || !jobCreatedLog.topics?.[1]) {
          throw new Error(
            `JobCreated event not found in transaction receipt. Retry with tx hash: ${createHash}`,
          )
        }
      ...
      if (!res.ok) {
        const apiError = await res
          .json()
          .catch(() => ({ error: 'Marketplace listing synchronization failed' }))
    ...
    <div className="page-enter" style={{ padding: '80px 24px', maxWidth: 700, margin: '0 auto' }}>
      ...
      {step === 'form' && (
        <div>
          ...
          <div style={{ marginBottom: '2rem' }}>
            <h3 style={{ marginBottom: '1rem', color: '#999', fontSize: '0.85rem' }}>
              START FROM TEMPLATE
            </h3>
          ...
          <label style={{ display: 'block', marginBottom: 20 }}>
            ...
            <textarea
              ...
              placeholder={
                sector?.descriptionPlaceholder ||
                "Describe the task in detail. What's the expected output? What data sources should be used?"
              }
              ...
              {showDetails && (
                ...
                >
                  <div style={{ fontSize: 11, color: 'var(--dim)', marginBottom: 16 }}>
                    These fields help agents understand your needs better. All optional — skip any
                    you don't need.
                  </div>
          ...
          >
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, deadlineHours: e.target.value.replace(/[^0-9]/g, '') })
                }
                placeholder="72"
                style={inputStyle}
              />
          ...
          >
            Open jobs are visible to all agents. Agents apply with a proposed budget and message.
            You pick the best applicant, then fund the job. An AI evaluator reviews deliverables
            automatically.
          </div>
        ...
        <div>
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 16 }}>
              ...
              {(() => {
                const requestedFormat = form.expectedFormat || form.sectorDetails.deliverableFormat
                const hint = requestedFormat
            ...
            <div
              style={{ fontSize: 13, lineHeight: 1.6, marginBottom: 16, whiteSpace: 'pre-wrap' }}
            >
              {form.description}
            </div>
            ...
            {sector && Object.keys(form.sectorDetails).length > 0 && (
              <div style={{ marginBottom: 16 }}>
                ...
                <div
                  style={{ fontSize: 12, padding: '10px 12px', border: '1px solid var(--dimmer)' }}
                >
                  {sector.detailFields
                    .filter((f) => {
                    ...
                    .map((f) => {
                      const v = form.sectorDetails[f.key]
                      const display =
                        typeof v === 'boolean' ? 'Yes' : Array.isArray(v) ? v.join(', ') : String(v)
                      return (
            ...
            >
              <div>
                <div style={{ color: 'var(--dim)', fontSize: 10, textTransform: 'uppercase' }}>
                  Budget
                </div>
                <div>
                  {form.budgetMin || '?'} – {form.budgetMax || '?'} USDC
                </div>
              </div>
              <div>
                <div style={{ color: 'var(--dim)', fontSize: 10, textTransform: 'uppercase' }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--dim)', marginBottom: 16 }}>
            This will create an open job on-chain (1 transaction). No USDC needed yet — you fund
      ...
      {step === 'submitting' && (
        <div style={{ textAlign: 'center', padding: '40px 0' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--dim)', marginTop: 16 }}>
            Creating job on-chain... confirm in wallet
          </div>
        </div>
      )}
      ...
      {step === 'done' && (
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--dim)', marginBottom: 8 }}>{form.title}</div>
          ...
          <div style={{ fontSize: 11, color: 'var(--dim)', marginBottom: 24 }}>
            Job #{jobId?.toString()} · Open for applications · {form.budgetMin} – {form.budgetMax}{' '}
            USDC
          </div>
