...
export default function Dashboard() {
  ...
  async function fetchAll() {
    ...
    try {
      ...
      // Keep the previously loaded dashboard panels when refresh fails.
    }
    setLoading(false)
  }
  ...
  if (!isConnected) {
    return (
      ...
      >
        ...
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 12 }}>Connect your wallet</div>
        <div
          style={{
            fontSize: 12,
            color: 'var(--dim)',
    ...
    >
      ...
      {walletStats && (
        ...
        >
          ...
          <StatCard
            label="My Active"
            value={(walletStats.activeAsClient + walletStats.activeAsProvider).toLocaleString()}
          />
          <StatCard
            label="My Completed"
            value={(
              walletStats.completedAsClient + walletStats.completedAsProvider
            ).toLocaleString()}
          />
          <StatCard
            label="My Volume"
      ...
      >
        ...
        >
          ...
          <span style={{ color: 'var(--code-text)' }}>
            npm install <span style={{ color: 'var(--accent)' }}>@archivee/agent</span>
          </span>
        </div>
        <Link
          to="/docs"
      ...
      >
        <button
          ...
          style={{
            padding: '10px 20px',
            fontSize: 12,
            fontWeight: dashTab === 'jobs' ? 700 : 400,
        ...
        <button
          ...
          style={{
            padding: '10px 20px',
            fontSize: 12,
            fontWeight: dashTab === 'earnings' ? 700 : 400,
            background: 'transparent',
            border: 'none',
            cursor: 'pointer',
            color: dashTab === 'earnings' ? 'var(--text)' : 'var(--dim)',
            borderBottom:
              dashTab === 'earnings' ? '2px solid var(--accent)' : '2px solid transparent',
            marginBottom: -1,
          }}
        >
          EARNINGS
      ...
      {dashTab === 'earnings' ? (
        ...
        <div>
          ...
          >
            <div style={{ border: '1px solid var(--dimmer)', padding: '1rem' }}>
              <div style={{ color: 'var(--dim)', fontSize: '0.75rem', marginBottom: '0.25rem' }}>
                TOTAL EARNED
              </div>
              <div
                style={{ color: 'var(--accent)', fontSize: '1.5rem', fontFamily: 'var(--font)' }}
              >
            ...
            <div style={{ border: '1px solid var(--dimmer)', padding: '1rem' }}>
              ...
              <div style={{ color: 'var(--text)', fontSize: '1.5rem', fontFamily: 'var(--font)' }}>
                ...
                {walletStats?.completedAsProvider
                  ? (
                      (walletStats.earned ? parseFloat(walletStats.earned) : 0) /
                      walletStats.completedAsProvider
                    ).toFixed(0)
                  : '0'}
              </div>
        ...
        <div>
          ...
          {loading ? (
            ...
            <div
              style={{ textAlign: 'center', padding: '60px 0', color: 'var(--dim)', fontSize: 12 }}
            >
              {tab === 'open' ? 'No active jobs.' : 'No completed jobs yet.'}
              {tab === 'open' && (
                <div style={{ marginTop: 12 }}>
                  <Link to="/post-job" style={{ color: 'var(--accent)', fontSize: 12 }}>
                    + Post a job
                  </Link>
                </div>
              )}
...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ padding: '12px 14px', border: '1px solid var(--dimmer)' }}>
      <div
        style={{ fontSize: 10, color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: 1 }}
      >
        {label}
      </div>
