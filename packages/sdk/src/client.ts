...
export class HttpClient {
  ...
  private async requestWithRetry<T>(
    ...
    // GET is safe to retry. Marketplace writes are not generically idempotent:
    // a timeout can happen after the API stored an application/deliverable.
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      try {
        ...
        // A write can have reached the server before a timeout. Retrying it
        // would create a second application or deliverable version.
        if (!retryable) throw fail(error)
