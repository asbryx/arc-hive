...
export function hasCommerceEvent(
  receipt: ReceiptLike,
  eventName: 'JobCreated' | 'JobFunded' | 'JobCompleted',
  jobId: bigint,
  ...
  for (const log of receipt.logs) {
    ...
    try {
      ...
      const args = decoded.args as { jobId: bigint; client?: string }
      if (decoded.eventName !== eventName || args.jobId !== jobId) continue
      if ((eventName === 'JobCreated' || eventName === 'JobFunded') && args.client?.toLowerCase() !== clientAddress?.toLowerCase()) continue
      return true
    } catch {
      // unrelated log
    }
